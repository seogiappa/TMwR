...
21세기 초부터 수학적 모델은 명백하고 미묘한 방식으로 일상 생활에서 어디에나 존재하게 되었습니다. 많은 사람들의 일반적인 하루는 애완견을 산책시키기에 좋은 시간을 선택하기 위해 하루의 날씨를 확인하고, 웹사이트에서 제품을 주문하고, 친구에게 문자 메시지를 입력하고 자동 수정하고, 이메일을 확인하는 일이 포함될 수 있습니다. 이러한 각각의 경우에 어떤 유형의 수학적 모델이 관련되었을 가능성이 높습니다. 어떤 경우에는 이러한 모델이 사용됨을 쉽게 인식할 수도 있지만(예, '제품 _X_ 구매'), 그렇지 않은 경우도 있습니다(예: 스팸 이메일). 모델은 고객이 좋아할 만한 의복을 선택하고, 약물 후보로 평가되어질 분자를 식별하는 데 사용되며, 심지어 사악한 회사가 과도하게 오염시키는 자동차의 발견을 피하기 위해 사용하는 메커니즘일 수도 있습니다. 좋든 나쁘든 모델은 계속 존재합니다.


:::rmdnote
오늘날 모델이 우리 삶에 스며든 두 가지 이유가 있습니다. 모델을 생성하기 위한 풍부한 **소프트웨어**가 존재하고 **데이터**를 기록하고 접근하는 것이 쉬워졌기 때문입니다. 
:::
